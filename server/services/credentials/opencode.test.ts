...
describe('credentials/opencode adapter', () => {
  ...
  beforeEach(() => {
    tempRoot = fs.mkdtempSync(path.join(os.tmpdir(), 'ralphy-opencode-adapter-'));
  ...
  it('buildSdkEnv() tags RALPHY_USER_ID so the provider can pluck userId off options.env', () => {
    const env = openCodeCredentialStore.buildSdkEnv(99);
    expect(env['RALPHY_USER_ID']).toBe('99');
  });
