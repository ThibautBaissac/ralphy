...
//
//   ~/.config/ralphy/users/{userId}/opencode-data/opencode/auth.json
...
// `~/.config/ralphy/users/{userId}/opencode-data` — so the file above is
...
export interface OpenCodeSpawnEnv extends Record<string, string | undefined> {
   ...
   * `~/.ralphy/projects/.../task-*.md` (task docs) and the
   * `${HOME}/.config/ralphy/...` per-user state tree. Ralphy is the
