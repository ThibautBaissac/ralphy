 ...
 * Ralphy CLI
 *
 * Provides command-line utilities for managing Ralphy
 *
 * Commands:
...
function getDatabasePath(): string {
  loadEnvFile();
  return process.env.DATABASE_PATH || path.join(__dirname, 'database', 'ralphy.db');
}
...
function showStatus(): void {
  ...
  console.log(`      ${c.dim('>')} Run "ralphy" or "cloudcli start" to start the server`);
...
Commands:
  start          Start the Ralphy server (default)
  status         Show configuration and data locations
  help           Show this help information
  version        Show version information

Examples:
  $ ralphy                     # Start the server
...
Documentation:
  ${packageJson.homepage || 'https://github.com/vdaubry/ralphy'}

Report Issues:
  ${packageJson.bugs?.url || 'https://github.com/vdaubry/ralphy/issues'}
`);
}
