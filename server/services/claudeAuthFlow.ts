...
const AUTH_URL_REGEX = /https:\/\/(?:[A-Za-z0-9.-]*claude\.com|claude\.ai)\/[A-Za-z0-9._\-/?#=&%]+/g;
...
function extractAuthUrl(output: string): string | null {
  ...
  const cleaned = stripAnsiForUrl(output).replace(
    /(?<=[A-Za-z0-9._\-/?#=&%])([\r\n]+)(?=[A-Za-z0-9._\-/?#=&%])/g,
    (whole, brk: string) => ((brk.match(/\n/g) || []).length === 1 ? '' : whole),
  );
  const urls = [...cleaned.matchAll(AUTH_URL_REGEX)].map((match) => match[0]);
  if (urls.length === 0) return null;
  const url = urls.map(normalizeClaudeAuthUrl).find(isCompleteAuthUrl) || null;
  if (!url) return null;
  return url;
}

function normalizeClaudeAuthUrl(url: string): string {
  try {
    const parsed = new URL(url);
    const isClaudeAiManualCodeUrl =
      parsed.hostname === 'claude.ai' &&
      parsed.pathname === '/oauth/authorize' &&
      parsed.searchParams.get('code') === 'true' &&
      parsed.searchParams.get('redirect_uri') ===
        'https://platform.claude.com/oauth/code/callback';

    if (isClaudeAiManualCodeUrl) {
...
function isCompleteAuthUrl(url: string): boolean {
  try {
    const parsed = new URL(url);
    if (parsed.searchParams.get('code') === 'true') {
      ...
      return (
        ...
        Boolean(parsed.searchParams.get('scope')) &&
        parsed.searchParams.get('code_challenge_method') === 'S256' &&
        codeChallenge.length >= MIN_PKCE_VALUE_LENGTH &&
        state.length >= MIN_PKCE_VALUE_LENGTH
      );
    }
  } catch {
    return false;
  ...
  // Legacy/simple authorize URLs in tests only carry `state`. Modern Claude
  // Code PKCE URLs are validated above so we do not expose a truncated final
  // state value that fails after the user approves the consent screen.
...
function stripAnsiForUrl(text: string): string {
  // Cursor-right escapes are spacing instructions in normal terminal text, but
  // if they land inside a URL they split otherwise valid PKCE values.
  return stripAnsi(text.replace(/\x1b\[\d*C/g, ''));
}
...
export async function startClaudeAuthLogin(
  ...
  const child = spawnPtyProcess(claudeCli, ['setup-token'], {
    name: 'xterm-256color',
    cols: 1000,
    rows: 30,
  ...
  const handleOutput = (chunk: Buffer | string, streamName: 'stdout' | 'stderr'): void => {
    ...
    if (!session.authUrl) {
      ...
      if (url) {
        ...
        let codeChallengeLength: number | null = null;
        try {
          const parsed = new URL(url);
          urlHost = parsed.host;
          urlParamCount = Array.from(parsed.searchParams.keys()).length;
          stateLength = parsed.searchParams.get('state')?.length ?? null;
          codeChallengeLength = parsed.searchParams.get('code_challenge')?.length ?? null;
        } catch {
