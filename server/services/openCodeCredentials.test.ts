...
describe('openCodeCredentials', () => {
  ...
  beforeEach(() => {
    tempRoot = fs.mkdtempSync(path.join(os.tmpdir(), 'ralphy-opencode-creds-'));
  ...
  it('buildOpenCodeSpawnEnv sets per-user XDG dirs, preserves host gh config, and strips inherited OPENCODE_* keys', () => {
    ...
    // OPENCODE_CONFIG_CONTENT is reset to a Ralphy-owned inline config
    // (grants external_directory: allow so the `build` agent's `read`
    // tool doesn't hang on ralphy-internal paths). Inherited values
    // from the parent process must be stripped first; the assertion
    // is that the final value is what Ralphy computed, not what the
