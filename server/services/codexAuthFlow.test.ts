...
vi.mock('./codexCredentials.js', () => ({
  ...
  ensureCodexHomeDir: vi.fn((userId) => ({
    codexHome: `/home/test/.config/ralphy/users/${userId}/codex/`,
  })),
  buildCodexSdkEnv: vi.fn((userId) => ({
    PATH: '/usr/bin',
    HOME: '/home/test',
    CODEX_HOME: `/home/test/.config/ralphy/users/${userId}/codex/`,
  })),
  ...
  resolveCodexAuthJsonPath: vi.fn((u: number) => `/home/test/.config/ralphy/users/${u}/codex/auth.json`),
}));
    ...
    expect(ptySpawnMock).toHaveBeenCalledWith(
      ...
      expect.objectContaining({
        env: expect.objectContaining({
          CODEX_HOME: '/home/test/.config/ralphy/users/42/codex/',
        }),
      }),
    );
  });
