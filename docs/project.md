# Ralphy — Project Documentation

Ralphy is a web-based interface for the Claude Code CLI: a desktop/mobile-friendly UI for managing coding projects and driving Claude through structured agentic workflows.
...
│  Filesystem:                                                      │
│     ~/.ralphy/projects/{id}/tasks/task-{id}.md (task docs)       │
│     ~/.ralphy/prompts/*.md                    (user prompt       │
│                                                 overrides)        │
│     ~/.config/ralphy/users/{userId}/oauth_token (Claude OAuth)   │
