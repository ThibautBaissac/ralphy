 ...
 * Falls back to the default ("ralphy") if the table is unavailable.
 */
export function getConfiguredTrigger(): string {
  try {
    return appSettingsDb.getValue('github_pr_trigger') || 'ralphy';
  } catch {
    return 'ralphy';
  }
}
...
/**
 * Check if the configured @-trigger (e.g. "@ralphy") is mentioned in
