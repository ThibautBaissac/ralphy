...
//
// The orchestrator passes the user id via the env (RALPHY_USER_ID, tagged
...
function extractUserIdFromEnv(
  ...
  const raw = env?.['RALPHY_USER_ID'];
  const n = Number(raw);
  if (!Number.isInteger(n) || n <= 0) {
    throw new Error(
      `OpenCodeProvider could not resolve user id from env (RALPHY_USER_ID=${
...
interface ActiveOpenCodeSession {
   ...
   * the serve-time `process.cwd()` — the Ralphy worktree) and calls
...
async function* streamUnified(
  ...
  //    server's default bus (Ralphy's worktree), while `promptAsync`
  //    publishes the turn on the task-worktree bus — leaving Ralphy
  ...
  // question API, which Ralphy has no UI for (capability
  ...
  // fallback is the *server's* `process.cwd()` — i.e. the Ralphy
  ...
  // an implementation run wrote `hello.txt` into the ralphy worktree
  // and a planning run explored the ralphy codebase instead of the
...
export class OpenCodeProvider implements LlmProvider {
  ...
  abortTurn(providerSessionId: string): boolean {
    ...
    // only stops Ralphy's SSE listener, not the out-of-process turn.
    console.log(
...
/**
 * Ralphy-facing model record. Mirrors the subset of OpenCode's `Model`
 * type the settings UI needs (id, label, status, context window). The
 * `id` field is the *Ralphy-persisted* shape `opencode/<bareModelID>`
 ...
 * separate credentials Ralphy doesn't manage.
 *
