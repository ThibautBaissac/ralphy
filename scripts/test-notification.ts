...
const DB_PATH = process.env.DATABASE_PATH || path.join(__dirname, '../server/database/ralphy.db');

async function sendTestNotification(username: string): Promise<void> {
    ...
    try {
        ...
        console.error('❌ Failed to send banner notification:', message);
...
void sendTestNotification(username);
