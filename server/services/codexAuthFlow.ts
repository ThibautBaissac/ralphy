...
//      service and writes `$CODEX_HOME/auth.json` on success. Ralphy
...
// `CODEX_HOME=~/.config/ralphy/users/{userId}/codex/` and strips every
...
export async function startCodexAuthLogin(
  ...
  const child = spawnPtyProcess(codexCli, ['login', '--device-auth'], {
    name: 'xterm-256color',
    cols: 1000,
    rows: 30,
