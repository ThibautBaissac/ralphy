...
- **Database**: SQLite (better-sqlite3) at `server/database/ralphy.db`
...
```bash
sqlite3 server/database/ralphy.db "SELECT id, task_id, claude_conversation_id, session_path FROM conversations WHERE id = <ID>;"
```
...
sqlite3 server/database/ralphy.db "SELECT seq, json_extract(entry_json,'$.type') AS type FROM messages WHERE session_id = (SELECT claude_conversation_id FROM conversations WHERE id = <ID>) ORDER BY seq"
```
...
```bash
sqlite3 server/database/ralphy.db "SELECT * FROM conversations WHERE id = 2683;"
sqlite3 server/database/ralphy.db "SELECT * FROM tasks WHERE id = 562;"
