 ...
 *
 * Override with RALPHY_ARCHIVE_ROOT in tests.
 */
function getArchiveRoot(): string {
  return process.env.RALPHY_ARCHIVE_ROOT || path.join(os.homedir(), '.ralphy');
}
...
Note: any \`.ralphy/tasks/*.md\` files inside the repo itself are legacy from before task docs were moved to a central archive. Ignore them — the path above is the only source of truth.`);
