...
describe('Webhook Service', () => {
  beforeEach(() => {
    ...
    vi.mocked(mockGetAppSetting).mockReturnValue('ralphy');
  });
  ...
  describe('getConfiguredTrigger', () => {
    ...
    it('falls back to "ralphy" if the lookup returns falsy', () => {
      vi.mocked(mockGetAppSetting).mockReturnValue('');
      expect(getConfiguredTrigger()).toBe('ralphy');
    });

    it('falls back to "ralphy" if the database throws', () => {
  ...
  describe('hasTriggerMention', () => {
    it('detects the default @ralphy trigger from app_settings', () => {
      vi.mocked(mockGetAppSetting).mockReturnValue('ralphy');
      expect(hasTriggerMention('@ralphy please fix this')).toBe(true);
      expect(hasTriggerMention('Hey @ralphy, can you help?')).toBe(true);
    });

    it('is case-insensitive', () => {
      expect(hasTriggerMention('@Ralphy please fix', 'ralphy')).toBe(true);
      expect(hasTriggerMention('@RALPHY please fix', 'ralphy')).toBe(true);
      expect(hasTriggerMention('@RaLpHy please fix', 'ralphy')).toBe(true);
    });
    ...
    it('respects a custom trigger argument over the configured default', () => {
      ...
      expect(hasTriggerMention('@ralphy please fix', 'acme')).toBe(false);
    });

    it('returns false when the trigger is absent', () => {
      expect(hasTriggerMention('Please fix this bug', 'ralphy')).toBe(false);
      expect(hasTriggerMention('ralphy without at sign', 'ralphy')).toBe(false);
      expect(hasTriggerMention('', 'ralphy')).toBe(false);
    });

    it('returns false for null or undefined comment bodies', () => {
      expect(hasTriggerMention(null, 'ralphy')).toBe(false);
      expect(hasTriggerMention(undefined, 'ralphy')).toBe(false);
    });
  });
  ...
  describe('triggerPrAgentFromComment', () => {
    ...
    it('should trigger PR agent successfully', async () => {
      const result = await triggerPrAgentFromComment({
        taskId: 123,
        commentBody: '@ralphy please fix this',
    ...
    it('should include file context for inline review comments', async () => {
      ...
      const result = await triggerPrAgentFromComment({
        taskId: 123,
        commentBody: '@ralphy refactor this',
