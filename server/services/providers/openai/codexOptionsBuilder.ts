...
/**
 * Map Ralphy's PermissionMode onto Codex's sandbox + approval-policy
 * pair. Ported verbatim from `claudecodeui`'s
 * `mapPermissionModeToCodexOptions` (server/openai-codex.js:168-187).
 * Ralphy's runtime default is `bypassPermissions` (D8), so most
 ...
 * and Ralphy worktree paths aren't always git repos in the
 * conventional sense.
 */
