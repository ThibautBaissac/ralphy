...
describe('Documentation Service - Phase 2', () => {
  ...
  beforeEach(() => {
    testRepoPath = fs.mkdtempSync(path.join(os.tmpdir(), 'ralphy-test-'));
    archiveRoot = fs.mkdtempSync(path.join(os.tmpdir(), 'ralphy-archive-'));
  ...
  describe('writeTaskDoc', () => {
    ...
    it('should not write to the repo .ralphy/tasks folder', () => {
      writeTaskDoc(testProjectId, 1, 'Content');

      expect(fs.existsSync(path.join(testRepoPath, '.ralphy', 'tasks', 'task-1.md'))).toBe(false);
    });
  ...
  describe('buildContextPrompt', () => {
    ...
    it('should warn that in-repo .ralphy/tasks files are legacy', () => {
      const result = buildContextPrompt(testProjectId, 1);

      expect(result).toContain('.ralphy/tasks');
