...
const ENV_WITH_USER = { RALPHY_USER_ID: '7', XDG_DATA_HOME: '/tmp/fake/7/data' };
...
describe('OpenCodeProvider', () => {
  ...
  it('always disables `question` even when disallowedTools is empty (Ralphy has no UI to answer it)', async () => {
    const { handle, client } = makeFakeHandle([
      { type: 'session.idle', properties: { sessionID: 'sess_new_123' } },
    ]);
    vi.mocked(getOrSpawnOpenCodeServer).mockResolvedValue(handle as never);
  ...
  it('threads cwd through to event.subscribe as query.directory (OpenCode /event is workspace-scoped — without this the subscription lands on the default bus and session events never reach Ralphy)', async () => {
    const { handle, client } = makeFakeHandle([
      { type: 'session.idle', properties: { sessionID: 'sess_new_123' } },
    ]);
    vi.mocked(getOrSpawnOpenCodeServer).mockResolvedValue(handle as never);
  ...
  it('throws when the env does not carry RALPHY_USER_ID', async () => {
    const p = new OpenCodeProvider();
    await expect(
      p.startTurn({
        cwd: '/repo',
    ...
    ).rejects.toThrow(/RALPHY_USER_ID/);
  });
});
